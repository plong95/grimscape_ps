...
public class PlayerUpdating {
    ...
    private static void updateMovement(Player player, PacketBuilder builder) {
        ...
        if (player.isNeedsPlacement() || player.isChangingRegion()) {
             ...
             */
            builder.putBits(7, player.getPosition().getLocalY(player.getCurrentMapCenter()));
            builder.putBits(7, player.getPosition().getLocalX(player.getCurrentMapCenter()));
        } else /*
     ...
     *
     * @param builder The packet builder used to write sendInformation on.
