...
public class House extends RegionInstance {
     ...
     * p.getPacketSender().sendInteractionOption("null", 3, true); p.combatRingType
     * = 0; p.moveTo(new Position(ConstructionData.BASE_X+(myTiles[0]*8)+1,
     * ConstructionData.BASE_Y+(myTiles[1]*8)+1, p.getPosition().getZ())); } else {
     * PlayerFurniture portal = Construction.findNearestPortal(p); int toX =
