...
class Dungeoneering(val group: DungeoneeringParty) {
    ...
    fun startDungeon() {
        ...
        World.getNpcs().forEach(Consumer { n: NPC ->
            n.removeNpcs(
                Locations.Location.DUNGEONEERING_ROOM,
                height
            )
        })
    ...
    fun finalize() {
        ...
        group.players.forEach {
            ...
            it.packetSender.sendDungeoneeringTabIcon(true).sendTabInterface(GameSettings.QUESTS_TAB, 27224)
        ...
        group.players.forEach {
            ...
            if (!it.inventory.isFull) {
                ...
                GroundItemManager.spawnGroundItem(
                    it,
                    GroundItem(
                        Item(TOKENS, tokens), Constants.LOBBY.copy(),
    ...
    fun check() {
        if (!bossDead && !bossSpawned && minionKills >= KILLS_REQUIRED) {
            ...
            val bossNpc =
                DungeoneeringBossNpc(DungeoneeringBossNpc.Constants.BOSS_NOHEADICON, Position(2284, 5013, height))
