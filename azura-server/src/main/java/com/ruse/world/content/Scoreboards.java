...
public class Scoreboards {
    ...
    public static Scoreboard TOP_DONATION;
    public static Scoreboard TOP_KC;
    public static Scoreboard TOP_ACHIEVER;
    public static Scoreboard TOP_TOTAL_EXP;

    public static void init() {
        TOP_DONATION = new Scoreboard(GameSettings.RSPS_NAME + " Top Donators", "top-pkers.txt") {
            @Override
        ...
        TOP_KC = new Scoreboard(GameSettings.RSPS_NAME + " Top NPC KC", "top-killstreaks.txt") {
            @Override
            public void resort(ArrayList<ScoreboardEntry> entries) {
    ...
    public static ArrayList<ScoreboardEntry> getUpdatedList(Scoreboard scoreboard) {
        ...
        for (Player p : World.getPlayers()) {
            if (p == null)
                continue;
            if (scoreboard == Scoreboards.TOP_DONATION) {
                entries.add(new ScoreboardEntry(p.getUsername(),
                        new String[]{String.valueOf(p.getAmountDonated())}));
    ...
    public static void open(Player player, Scoreboard scoreboard) {
        ...
        for (int i = 0; i <= 50; stringId++, i++) {
            ...
            String line = "";
            if (entry != null) {
                if (scoreboard == Scoreboards.TOP_KC) {
                    line = "@whi@Rank @or1@" + (i + 1) + "@whi@ - " + entry.player.replaceAll("_", " ")
                            + " - NPC KC: @or1@" + Integer.valueOf(entry.value[0]);
                } else if (scoreboard == Scoreboards.TOP_DONATION) {
                    line = "@whi@Rank @or1@" + (i + 1) + "@whi@ - " + entry.player.replaceAll("_", " ")
                            + " - Top Donators: @or1@" + Integer.parseInt(entry.value[0]);
                } else if (scoreboard == Scoreboards.TOP_TOTAL_EXP) {
                    line = "@whi@Rank @or1@" + (i + 1) + "@whi@ - " + entry.player.replaceAll("_", " ")
        ...
        entries.clear();
        player.getPacketSender().sendInterface(6308).sendString(6400, "Scoreboard - " + scoreboard.title + "")
                .sendString(6399, "").sendString(6401, "Close");
    }
