package com.ruse.world.content.gamblinginterface;
...
/**
 * @author Suic for all the games/interface, etc.
 */
...
public class GamblingInterface {
    ...
    public void requestGamble(Player player2) {
        ...
        if (player.isGambleBanned()) {
            player.getPacketSender().sendMessage("@red@You are gamble banned and cannot do this.");
            return;
        }
        ...
        if (player.getAmountDonated() <= 99) {
            player.getPacketSender().sendMessage("@red@[Warning]@bla@ You need to donate a total of $100 to gamble - ::donate");
            return;
        }
        if (player2.getAmountDonated() <= 99) {
    ...
    private void giveItems() {
        ...
        for (Item item : player.getGambling().offeredItems) {
            player.getInventory().add(item);
            PlayerLogs.logPlayerGambles(player.getUsername(), "Got back item against " + player2.getUsername() + ". Name: "
                    + item.getDefinition().getName() + ". Id: " + item.getId() + ", amount: " + item.getAmount());
        }

        for (Item item : player2.getGambling().offeredItems) {
            player.getInventory().add(item);
            PlayerLogs.logPlayerGambles(player.getUsername(), "Won item against " + player2.getUsername() + ". Name: "
                    + item.getDefinition().getName() + ". Id: " + item.getId() + ", amount: " + item.getAmount());
            PlayerLogs.logPlayerGambles(player2.getUsername(), "Lost item against " + player.getUsername() + ". Name: "
                    + item.getDefinition().getName() + ". Id: " + item.getId() + ", amount: " + item.getAmount());
        }
    ...
    private void start55x2() {
        ...
        player.moveTo(new Position(2852, 2705));
        player2.moveTo(new Position(2852 - 2, 2705));
    ...
    private void startFlowerPoker() {
        ...
        player2.setPositionToFace(null);
        ...
        TaskManager.submit(new Task(6) {
            ...
            protected void execute() {
                ...
                plantFlower();
                player.getMovementQueue().addStep(new Position(player.getPosition().getX(), player.getPosition().getY() - 1, 0));
                player2.getMovementQueue().addStep(new Position(player2.getPosition().getX(), player2.getPosition().getY() - 1, 0));
    ...
    private void resetData() {
        ...
        player.getPacketSender().sendString(57157, "Description will be listed here")
                .sendString(57158, "It'll update after you choose a gamemode")
                .sendString(57159, "This is the 3rd line of description :D");
    }
