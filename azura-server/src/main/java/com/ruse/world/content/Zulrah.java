...
public class Zulrah {
    ...
    public static void enter(Player player) {
        ...
        NPC zulrah = new NPC(ZulrahLogic.phase[Misc.randomMinusOne(ZulrahLogic.phase.length)], new Position(
                ZulrahLogic.move[rand].getX(), ZulrahLogic.move[rand].getY(), player.getPosition().getZ()));
        ...
        TaskManager.submit(new Task(1, player, false) {
            ...
            protected void execute() {
                if (player == null || player.getConstitution() <= 0 || player.isDying() || zulrah == null
                        || zulrah.getConstitution() <= 0 || zulrah.isDying()) {
                    return;
                }
