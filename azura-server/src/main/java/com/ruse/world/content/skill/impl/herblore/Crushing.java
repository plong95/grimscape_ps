...
/**
 * Given life by Crimson on 5/14/2017. Made functional by crimson tho u fucking
 * goof
 */
public enum Crushing {
    ...
    public static void handleCrushing(Player player, int index) {
        ...
        player.setCurrentTask(new Task(1, player, false) {
            ...
            public void execute() {
                if (!player.getInventory().contains(pestle)
                        || !player.getInventory().contains(Crushing.values()[index].getInput())) {
                    stop();
                    return;
                }
                if (player.getInventory().isFull()
                        && ItemDefinition.forId(Crushing.values()[index].getInput()).isStackable()) {
