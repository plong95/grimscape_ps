...
public abstract class Spell {
    ...
    /**
     * Determines if this spell is able to be cast by the argued {@link Player}. We
     * do not include {@link Npc}s here since no checks need to be made for them
    ...
    public boolean canCast(Player player, boolean delete) {
        ...
        if (itemsRequired(player).isPresent()) {
            ...
            /*
             * if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 17293) {
             * //17293 PlayerMagicStaff.suppressRunes(player, itemsRequired(player).get());
            ...
            if (player.getEquipment().containsAny(PlayerMagicStaff.allstaves)) {
                for (int i = 0; i < items.length; i++) {
                    if (items[i] != null && itemsRequired(player).get()[i] != null && player.getInventory()
                            .getAmount(items[i].getId()) < itemsRequired(player).get()[i].getAmount()) {
                        player.getPacketSender()
                                .sendMessage("You don't have the required amount of runes to cast this spell.");
                        player.getCombatBuilder().reset(true);
                        return false;
                    }
                }
     ...
     * @return the items required to cast this spell, or <code>null</code> if there
     * are no items required.
     */
     ...
     * @return the equipment required to cast this spell, or <code>null</code> if
