...
public class christmas2016 {
        ...
        if (itemId == 1907) { // mind bomb
            ...
            if (player.getChristmas2016() >= 3) {
                player.getPacketSender().sendMessage(
                        "You've already given the Reindeer their " + ItemDefinition.forId(itemId).getName() + ".");
                return;
                ...
                player.getPacketSender()
                        .sendMessage("You put the " + ItemDefinition.forId(itemId).getName() + " under the Reins.");
            } else {
                player.getPacketSender().sendMessage(
                        "You need at least 100 " + ItemDefinition.forId(itemId).getName() + " for the Reindeer.");
            }
    ...
    /*
     * public static void announceChristmas() { final NPC npc = new NPC(1552, new
     * Position(3674, 2981, 0)); if (npc == null || ) { //npc.setPosition(new
     * Position(2674, 2981, 0)); npc.setPositionToFace(new Position(3675, 2981, 0));
     * World.register(npc); } if (npc.getPosition().getX() != 3674 ||
     * npc.getPosition().getY() != 2981) { return; } else {
