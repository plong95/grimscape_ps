...
public class LightSupremeCombatScript implements CombatStrategy {
    ...
    public boolean customContainerAttack(Character entity, Character victim) {
        ...
        } else if (rand < 90 && !supreme.isChargingAttack()) {
        ...
        } else if (rand <= 100 && !supreme.isChargingAttack()) {
            supreme.performAnimation(anim);
            supreme.forceChat("Dodge this!!");
            supreme.setChargingAttack(true);
            for (int i = 0; i < 50; i++) {
                int randX = Misc.random(2761, 2806), randY = Misc.random(4566, 4586);
