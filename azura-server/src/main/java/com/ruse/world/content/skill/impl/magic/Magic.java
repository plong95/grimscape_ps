...
public class Magic {
    ...
    private static boolean hasRunes(Player player, int spellID) {
        ...
        if (ens.getReq3() == 0) {
            return player.getInventory().contains(ens.getReq1())
                    && player.getInventory().getAmount(ens.getReq1()) >= ens.getReqAmt1()
                    && player.getInventory().contains(ens.getReq2())
                    && player.getInventory().getAmount(ens.getReq2()) >= ens.getReqAmt2()
                    && player.getInventory().contains(ens.getReq3())
                    && player.getInventory().getAmount(ens.getReq3()) >= ens.getReqAmt3();
        } else {
            return player.getInventory().contains(ens.getReq1())
                    && player.getInventory().getAmount(ens.getReq1()) >= ens.getReqAmt1()
                    && player.getInventory().contains(ens.getReq2())
    ...
    public enum Enchant {
        ...
        SAPPHIRERING(1637, 2550, 7, 18, 719, 114, 1, 1), SAPPHIREAMULET(1694, 1727, 7, 18, 719, 114, 1, 1),
        SAPPHIRENECKLACE(1656, 3853, 7, 18, 719, 114, 1, 1), SAPPHIREBOLTS(9337, 9240, 7, 18, 712, 238, 1, 10),

        EMERALDRING(1639, 2552, 27, 37, 719, 114, 2, 1), EMERALDAMULET(1696, 1729, 27, 37, 719, 114, 2, 1),
        EMERALDNECKLACE(1658, 5521, 27, 37, 719, 114, 2, 1), EMERALDBOLTS(9338, 9241, 27, 37, 712, 238, 2, 10),

        RUBYRING(1641, 2568, 47, 59, 720, 115, 3, 1), RUBYAMULET(1698, 1725, 47, 59, 720, 115, 3, 1),
        RUBYNECKLACE(1660, 11194, 47, 59, 720, 115, 3, 1), RUBYBOLTS(9339, 9242, 27, 59, 712, 238, 3, 10),

        DIAMONDRING(1643, 2570, 57, 67, 720, 115, 4, 1), DIAMONDAMULET(1700, 1731, 57, 67, 720, 115, 4, 1),
        DIAMONDNECKLACE(1662, 11090, 57, 67, 720, 115, 4, 1), DIAMONDBOLTS(9340, 9243, 27, 67, 712, 238, 4, 10),

        DRAGONSTONERING(1645, 22045, 68, 78, 721, 116, 5, 1), DRAGONSTONEAMULET(1702, 1712, 68, 78, 721, 116, 5, 1),
        DRAGONSTONENECKLACE(1664, 11113, 68, 78, 721, 116, 5, 1), DRAGONBOLTS(9341, 9244, 27, 78, 712, 238, 5, 10),

        ONYXRING(6575, 6583, 87, 97, 721, 452, 6, 1), ONYXAMULET(6581, 6585, 87, 97, 721, 452, 6, 1),
        ONYXNECKLACE(6577, 11128, 87, 97, 721, 452, 6, 1), ONYXBOLTS(9342, 9245, 27, 97, 712, 238, 6, 10);
        ...
        private Enchant(int unenchanted, int enchanted, int levelReq, int xpGiven, int anim, int gfx,
                        int reqEnchantmentLevel, int amount) {
