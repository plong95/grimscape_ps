...
public class ClanChatManager {
    ...
    public static void updateList(ClanChat clan) {
        clan.getMembers().sort(new Comparator<Player>() {
            @Override
        ...
        for (Player memberPlayer : clan.getMembers()) {
            if (memberPlayer != null) {
                if (memberPlayer.getRelations().getIgnoreList().contains(player.getLongUsername()))
                    continue;
                int img = player.getRights() == PlayerRights.ZENYTE_DONATOR ? 1508 : player.getRights().ordinal();
                int ironimg = -1;
                if (player.getGameMode() == GameMode.IRONMAN) {
                    ironimg = 840;
                } else if (player.getGameMode() == GameMode.ULTIMATE_IRONMAN) {
                    ironimg = 839;
                } else if (player.getGameMode() == GameMode.VETERAN_MODE) {
                    ironimg = 838;
    ...
    private static void join(Player player, ClanChat clan) {
        ...
        player.getPacketSender().sendMessage(" You're now talking in " + clan.getOwnerName() + "'s clan chat.");
