...
public class ItemDegrading {
    ...
    public static boolean handleItemDegrading(Player p, DegradingItem d) {
        ...
        if (equipId == d.nonDeg || equipId == d.deg) {
            ...
            if (!d.dust && currentCharges == 0) {
                ...
                p.getPacketSender()
                        .sendMessage("Your " + ItemDefinition.forId(equipId).getName() + " has fully depleted!");
                return true;
            }
            if (equipId == d.deg && !degradeCompletely && !d.dust) {
                return true;
    ...
    public static int getAndIncrementCharge(Player p, DegradingItem d, boolean reset) {
        switch (d) {
            ...
            case MORRIGANS_LEATHER_CHAPS:
                ...
                /*
                 * case BRAWLING_GLOVES_COOKING: case BRAWLING_GLOVES_FIREMAKING: case
                 * BRAWLING_GLOVES_FISHING: case BRAWLING_GLOVES_HUNTER: case
                 * BRAWLING_GLOVES_MINING: case BRAWLING_GLOVES_PRAYER: case
                 * BRAWLING_GLOVES_SMITHING: case BRAWLING_GLOVES_THIEVING: case
                 * BRAWLING_GLOVES_WOODCUTTING: int index = d.ordinal() - 15; if(reset) { return
                 * p.getBrawlerChargers()[index] = 0; } else { return
                 * p.getBrawlerChargers()[index]++; }
                 */
            case RING_OF_RECOIL:
                if (reset) {
    ...
    public enum DegradingItem {
        ...
        // true),
        // STATIUS_PLATEBODY(13884, 13886, Equipment.BODY_SLOT, maxAncientCharges, true,
        // true),
        // STATIUS_PLATELEGS(13890, 13892, Equipment.LEG_SLOT, maxAncientCharges, true,
        // true),
        // STATIUS_WARHAMMER(13902, 13904, Equipment.WEAPON_SLOT, maxAncientCharges,
        // true, true),

        // Vesta's equipment

        // VESTAS_CHAINBODY(13887, 13889, Equipment.BODY_SLOT, maxAncientCharges, true,
        // true),
        // VESTAS_PLATESKIRT(13893, 13895, Equipment.LEG_SLOT, maxAncientCharges, true,
        // true),
        // VESTAS_LONGSWORD(13899, 13901, Equipment.WEAPON_SLOT, maxAncientCharges,
        // true, true),
        // VESTAS_SPEAR(13905, 13907, Equipment.WEAPON_SLOT, maxAncientCharges, true,
        // true),

        // Zuriel's equipment

        // ZURIELS_HOOD(13864, 13866, Equipment.HEAD_SLOT, maxAncientCharges, true,
        // true),
        // ZURIELS_ROBE_TOP(13858, 13860, Equipment.BODY_SLOT, maxAncientCharges, true,
        // true),
        // ZURIELS_ROBE_BOTTOM(13861, 13863, Equipment.LEG_SLOT, maxAncientCharges,
        // true, true),
        // ZURIELS_STAFF(13867, 13869, Equipment.WEAPON_SLOT, maxAncientCharges, true,
        // true),

        // Morrigan's equipment

        // MORRIGANS_COIF(13876, 13878, Equipment.HEAD_SLOT, maxAncientCharges, true,
        // true),
        // MORRIGANS_LEATHER_BODY(13870, 13872, Equipment.BODY_SLOT, maxAncientCharges,
        // true, true),
        MORRIGANS_LEATHER_CHAPS(7142, 7142, Equipment.LEG_SLOT, maxAncientCharges, true, true),
        ...
        public final static ArrayList<DegradingItem> getNonWeapons() {
            ...
            for (DegradingItem d : DegradingItem.values()) {
                if (d.getSlot() != Equipment.WEAPON_SLOT) {
                    // // System.out.println("Added "+d.toString().toLowerCase()+" to non-weapons
                    // array.");
                    values.add(d);
                }
            }
