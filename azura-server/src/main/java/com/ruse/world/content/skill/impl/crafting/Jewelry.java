...
public class Jewelry {
    ...
    public static void stringAmulet(Player player, final int itemUsed, final int usedWith) {
        ...
        if (!player.getInventory().contains(1759)) {
            player.getPacketSender().sendMessage("You need a ball of wool in order to string your "
                    + ItemDefinition.forId(amuletId).getName().toLowerCase() + ".");
            return;
        }
            ...
            case "RING":
                for (int i = 0; i < jewelryData.RINGS.item.length; i++) {
                    if (itemId == jewelryData.RINGS.item[i][1]) {
                        mouldJewelry(player, jewelryData.RINGS.item[i][0], itemId, amount, jewelryData.RINGS.item[i][2],
            ...
            case "NECKLACE":
                for (int i = 0; i < jewelryData.NECKLACE.item.length; i++) {
                    if (itemId == jewelryData.NECKLACE.item[i][1]) {
                        mouldJewelry(player, jewelryData.NECKLACE.item[i][0], itemId, amount,
                                jewelryData.NECKLACE.item[i][2], jewelryData.NECKLACE.item[i][3]);
                    }
                }
                break;
            case "AMULET":
                for (int i = 0; i < jewelryData.AMULETS.item.length; i++) {
                    if (itemId == jewelryData.AMULETS.item[i][1]) {
                        mouldJewelry(player, jewelryData.AMULETS.item[i][0], itemId, amount, jewelryData.AMULETS.item[i][2],
    ...
    private static void mouldJewelry(Player player, final int required, final int itemId, final int amount,
        ...
        for (final jewelryData i : jewelryData.values()) {
            ...
            if (player.getInventory().contains(1597)) {
                ...
                for (int j = 0; j < i.item.length; j++) {
                    ...
                    /*
                     * else { player.getPacketSender().sendItemOnInterface(4239,
                     * jewelryData.NECKLACE.item[j][0], j, 1); }
                     */
            ...
            if (player.getInventory().contains(1595)) {
                ...
                for (int j = 0; j < i.item.length; j++) {
                    ...
                    /*
                     * else { player.getPacketSender().sendItemOnInterface(4245,
                     * jewelryData.AMULETS.item[j][0], j, 1); }
                     */
    ...
    public static enum jewelryData {
        ...
        RINGS(new int[][]{{2357, 1635, 5, 15}, {1607, 1637, 20, 40}, {1605, 1639, 27, 55},
                {1603, 1641, 34, 70}, {1601, 1643, 43, 85}, {1615, 1645, 55, 100}, {6573, 6575, 67, 115}}),
        NECKLACE(new int[][]{{2357, 1654, 6, 20}, {1607, 1656, 22, 55}, {1605, 1658, 29, 60},
                {1603, 1660, 40, 75}, {1601, 1662, 56, 90}, {1615, 1664, 72, 105}, {6573, 6577, 82, 120}}),
        AMULETS(new int[][]{{2357, 1673, 8, 30}, {1607, 1675, 24, 65}, {1605, 1677, 31, 70},
                {1603, 1679, 50, 85}, {1601, 1681, 70, 100}, {1615, 1683, 80, 150}, {6573, 6579, 90, 165}});
    ...
    public static enum amuletData {
        GOLD(1673, 1692), SAPPHIRE(1675, 1694), EMERALD(1677, 1696), RUBY(1679, 1698), DIAMOND(1681, 1700),
        DRAGONSTONE(1683, 1702), ONYX(6579, 6581);
