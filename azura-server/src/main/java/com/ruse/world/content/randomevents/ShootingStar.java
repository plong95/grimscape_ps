    ...
    public static void sequence() {
        if (CRASHED_STAR == null) {
            if (timer.elapsed(TIME) || firstTime) {
                ...
                World.sendMessage(
                        "<img=5> <shad=1><col=FF9933>A shooting star has just crashed " + locationData.clue + "!");
                // new DiscordManager(DiscordConstant.EVENTS_CHANNEL, "Shooting Star", "A
                // shooting star has just crashed "+locationData.clue+"!").log1();
                World.getPlayers().forEach(p -> PlayerPanel.refreshPanel(p));
                // World.getPlayers().forEach(p -> p.getPacketSender().sendString(39161,
                // "@or2@Crashed star:
    ...
    public static void despawn(boolean respawn) {
        ...
        if (CRASHED_STAR != null) {
            for (Player p : World.getPlayers()) {
                ...
                // p.getPacketSender().sendString(39161, "@or2@Crashed star: @or2@[
                // @yel@N/A@or2@ ]");
            ...
            // new DiscordManager(DiscordConstant.EVENTS_CHANNEL, "Shooting Star", "The
            // shooting star has despawned.").log1();
