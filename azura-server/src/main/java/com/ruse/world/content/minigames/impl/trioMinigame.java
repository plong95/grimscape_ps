            ...
            public void execute() {
                ...
                if (player.getInventory().contains(11180)) {
                    ...
                    player.getPacketSender()
                            .sendMessage("You've spent 1 minute in the trio arena, one of your tokens were destroyed!");
                } else {
                    player.getCombatBuilder().cooldown(true);
                    player.getMovementQueue().reset();
                    player.moveTo(new Position(2902, 5204)); // adam change this to where ever the npc you talk to to
                    // start minigame is.
                    player.getMinigameAttributes().getTrioAttuibutes().setJoinedBossRoom(false);
