...
public class ToxicBlowpipe {
    ...
    public static boolean loadPipe(Player player) {
        int max = ItemDegrading.maxZulrahCharges, zscale = 12934, blowpipe = 12926,
                invcount = player.getInventory().getAmount(zscale), storecount = player.getBlowpipeCharges();
        ...
        if ((invcount + storecount) < max) {
            ...
            player.getPacketSender().sendMessage("You add " + Misc.format(invcount) + " and now have "
                    + Misc.format(player.getBlowpipeCharges()) + " total scale charges.");
            return true;
        }
...
/*
 * public enum dartTypes { BRONZE_DART(806), IRON_DART(807), STEEL_DART(808),
 * MITHRIL_DART(809), ADAMANT_DART(810), RUNE_DART(811), DRAGON_DART(11230);
 *
 *
 * private dartTypes(int dartId) { this.dartId = dartId; }
 *
 * public int dartId;
 *
 *
 ...
 *
 * public static dartTypes forId(int dartId2) { for(dartTypes dt :
 * dartTypes.values()) { if(dt.getDartId() == dartId2) { return dt; } } return
 * null; }
 *
 * }
 *
