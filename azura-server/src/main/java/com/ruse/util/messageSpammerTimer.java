...
/**
 * Created by Crim & overhauled on 5/06/2016. Now uses Yell_Bot_Prefix, counts
 * and resets automatically VS the old random system.
 */
public class messageSpammerTimer {

    private static int count = 0;
    private static long massMessageTimer = System.currentTimeMillis();
    private static String prefix = "";// <img=94><col=147939>News:
    private static boolean doubleExp = false;

    public static void massMessageHandler() {

        String[] messages = {
                // h'ween "@or1@Happy Halloween from Necrotic! Pumpkins & Scythes drop this
                // weekend!",
                // xmas "@red@Merry @gre@Christmas <col=FF0077>from Necrotic! Help Santa via the
                // Snomwan at home.",
                // xmas "@red@Sleighs are dropping for Christmas. @gre@They'll be gone
                // January.",
                // friday 13th "<col=1C1919><shad=ffffff>Try the *Friday the 13th* even through
                // the red portal @ ::home"
        ...
        if (System.currentTimeMillis() - massMessageTimer > GameSettings.massMessageInterval) {
            ...
            if (count == 10) {
                if (World.getPlayers().size() > 0) {
                    DiscordMessager.sendInGameMessage(":innocent: There is currently " + World.getPlayers().size() + " "
                            + (World.getPlayers().size() == 1 ? "player" : "players") + " online!");
                }
                if (doubleExp && Misc.isWeekend()) {
                    DiscordMessager.sendInGameMessage(
                            ":mega: Come get some **DOUBLE EXP**, it's here all weekend long and __STACKS__ with vote scrolls! :lifter:");
                }
                doubleExp = !doubleExp;
            }
