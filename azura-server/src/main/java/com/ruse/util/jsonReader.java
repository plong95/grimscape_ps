...
public class jsonReader {
    ...
    private static void handleJson(Path path) {
        ...
        try (FileReader fileReader = new FileReader(file)) {
            ...
            if (reader.has("total-play-time-ms") && reader.get("total-play-time-ms").getAsLong() >= 3600000
                    && reader.has("difficulty")) {
                count++;
                // System.out.println(
                //	count + "," + Misc.getTimeDurationBreakdown(reader.get("total-play-time-ms").getAsLong()) + ","
                //			+ Misc.capitalizeString(reader.get("difficulty").getAsString()) + ","
                //			+ reader.get("username").getAsString());
            }

            /*
             * String difficulty = builder.fromJson(reader.get("difficulty"), String.class);
             * int id = reader.get("total-play-time-msd").getAsInt();
             * // System.out.println("difficulty = "+difficulty+", playtime = "+id); difficulty
             * = null;
             */
