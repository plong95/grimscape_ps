...
/**
 * Represents a player's DungeoneeringIronEquipment item container.
 *
 * @author relex lawl
 */

public class DungeoneeringIronEquipment extends ItemContainer {
    ...
    /**
     * The DungeoneeringIronEquipment constructor.
     *
     * @param player The player who's DungeoneeringIronEquipment is being
     *               represented.
     */
    public DungeoneeringIronEquipment(Player player) {
        super(player);
    }
    ...
    public static int getItemCount(Player p, String s, boolean inventory) {
        int count = 0;
        for (Item t : p.getDungeoneeringIronEquipment().getItems()) {
    ...
    @Override
    public DungeoneeringIronEquipment full() {
        return this;
    }

    public DungeoneeringIronEquipment open() {
        getPlayer().getMovementQueue().reset();
        // getPlayer().getPacketSender().sendMessage("").sendMessage("Note: When selling
        // an item to a store, it loses 15% of its original value!").sendMessage("Prices
        // shown in the price-checker are the original values!");
        refreshItems();
        // open = true;
        return this;
    }
