...
public class GameObject extends Entity {
    ...
    public static long makeUid(int level, int x, int y, int type) {
        long uid = 0;
        uid |= (long) type << 40L;
        uid |= (long) level << 28L;
        uid |= (long) x << 14L;
    ...
    public static void addPrivateObject(Player player, GameObject object) {
        if (object.getId() == -1) {
            deletePrivateObject(player, object);
        } else {
            player.getPacketSender().sendObject(object);
        }
    }

    public static void deletePrivateObject(Player p, GameObject object) {
    ...
    public long getUid() {
        return makeUid(position.getZ(), position.getX(), position.getY(), type);
    }
