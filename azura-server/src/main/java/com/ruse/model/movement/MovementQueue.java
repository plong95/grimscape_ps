    ...
    public void addStep(Position step) {
        ...
        for (int i = 0; i < max; i++) {
            ...
            if (deltaY < 0)
                ...
                deltaY--;
            Position newPosition = new Position(x, y, step.getZ());
            if (character.isPlayer() && !character.asPlayer().getControllerManager().addWalkStep(last.position.getX(), last.position.getY(), newPosition.getX(), newPosition.getY())) {
                return;
            }
    ...
    public void sequence() {
        ...
        if (movement) {
            ...
            if (walkPoint != null && walkPoint.direction != Direction.NONE) {
                if (character.isPlayer() && !character.asPlayer().getControllerManager().canMove(walkPoint.direction.toInteger())) {
                    return;
                }
