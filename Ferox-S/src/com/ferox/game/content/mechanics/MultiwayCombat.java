...
public class MultiwayCombat {
    ...
    public static void refresh(Player player, int lastregion, int lastChunk) {
        ...
        if(player.raidsParty != null) {
            boolean raidsRegions = player.tile().region() == 4919 || player.tile().region() == 12889 || player.tile().region() == 13136 || player.tile().region() == 13137 || player.tile().region() == 13138 || player.tile().region() == 13139;
            if (!raidsRegions) {
