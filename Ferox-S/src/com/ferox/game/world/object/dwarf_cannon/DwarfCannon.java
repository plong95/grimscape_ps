...
public class DwarfCannon extends OwnedObject {
    ...
    public void checkDecayTimer() {
        ...
        if (needsDestroyed()) {
            getOwnerOpt().ifPresent(player ->  {
                player.message("<col=ff0000>Your cannon has decayed. Speak to Drunken dwarf to get a new one!</col>");
    ...
    public boolean handleAreaRestriction() {
        ...
        if(player.isInsideRaids()) {
            player.message("You can't place a cannon in Chambers of Secrets.");
            return false;
        }
    ...
    public static void onLogin(Player player) {
        ...
        if (reclaim) {
            player.message(Color.RED.wrap("Your cannon has been destoryed, you can reclaim it from the Drunken Dwarf at home."));
        }
    }
